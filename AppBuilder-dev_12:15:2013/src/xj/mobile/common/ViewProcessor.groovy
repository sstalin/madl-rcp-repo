	...
	if (widget && attr) { 
	  if (attr instanceof List) { 
		...
		if (attr.every{ widget[it] != null }) { 
		  def code = generator.generateSetCompoundAttributeCode(widget.widgetType, name, 
		...
		if (src && isDependentAttribute(widget, attr)) { 
		  ...
		  if (widget[attr]) { 
			def code = generator.generateSetAttributeCode(widget.widgetType, name, attr, widget[attr])
			if (code != null)
			  generator.injectCodeFromTemplate(classModel, CodeGenerator.InjectionPoint.LoadView, code[1] + ';')
		  }
		}
