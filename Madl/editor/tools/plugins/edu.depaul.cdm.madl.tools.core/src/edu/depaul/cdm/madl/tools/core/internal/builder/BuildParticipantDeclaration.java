...
public class BuildParticipantDeclaration {
   ...
   * {@link MadlBuildParticipant}. Instances of {@link MadlBuildParticipant} are shared across all
   * builders while instances of {@link BuildParticipant} are created one per project and wrapped in
   * new instances of {@link MadlBuildParticipant}. It is the responsibility of the caller to cache
    ...
    synchronized (lock) {
      ...
      while (iter.hasNext()) {
        ...
        try {
          ...
          MadlCore.logError("Exception instantiating build participant\n  from plugin "
