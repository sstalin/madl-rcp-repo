...
import edu.depaul.cdm.madl.tools.core.builder.BuildVisitor;
...
public class AnalysisEngineParticipant implements BuildParticipant {
  ...
  public AnalysisEngineParticipant() {
    this(MadlCore.getProjectManager(), AnalysisMarkerManager.getInstance());
  }
  ...
  public void clean(CleanEvent event, IProgressMonitor monitor) throws CoreException {
    ...
    event.getProject().deleteMarkers(MadlCore.MADL_PROBLEM_MARKER_TYPE, true, DEPTH_INFINITE);
    event.getProject().deleteMarkers(MadlCore.MADL_TASK_MARKER_TYPE, true, DEPTH_INFINITE);
  }
