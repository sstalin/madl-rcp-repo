 ...
 * copied out of <code>edu.depaul.cdm.madl.compiler.parser.MadlScanner</code> so that we could implement
 * {@link #validateIdentifier()}.
 * <p>
 * TODO the private methods in MadlScanner should be accessible so that we don't make copies here.
 *
 * @coverage madl.tools.core.generator
 */
public class MadlIdentifierUtil {

  /**
   * Given a string, return an equivalent string that is a valid Madl identifier.
   *
   * @param str
   * @return
   */
  ...
  public static IStatus validateIdentifier(final String str) {
    ...
    if (str == null || str.length() == 0) {
      return new Status(IStatus.ERROR, MadlCore.PLUGIN_ID, "This Madl identifier is empty.");
    }
    ...
    if (!isIdentifierStart(str.charAt(0))) {
      return new Status(
          IStatus.ERROR,
          MadlCore.PLUGIN_ID,
          "The first character of this Madl identifer is not a valid character, Madl identifiers "
    ...
    for (int i = 1; i < str.length(); i++) {
      if (!isIdentifierPart(str.charAt(i))) {
        return new Status(
            IStatus.ERROR,
            MadlCore.PLUGIN_ID,
            "This Madl identifer has an invalid character, identifiers "
