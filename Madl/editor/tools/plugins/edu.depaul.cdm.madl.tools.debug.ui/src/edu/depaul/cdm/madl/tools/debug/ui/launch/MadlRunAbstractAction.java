...
import edu.depaul.cdm.madl.tools.debug.core.MadlDebugCorePlugin;
import edu.depaul.cdm.madl.tools.debug.core.MadlLaunchConfigWrapper;
import edu.depaul.cdm.madl.tools.debug.ui.internal.DebugErrorHandler;
import edu.depaul.cdm.madl.tools.debug.ui.internal.DebugInstrumentationUtilities;
import edu.depaul.cdm.madl.tools.debug.ui.internal.util.LaunchUtils;
 ...
 */
public abstract class MadlRunAbstractAction extends InstrumentedAction implements
  ...
  private static class LaunchConfigComparator implements Comparator<ILaunchConfiguration> {
    ...
    public int compare(ILaunchConfiguration o1, ILaunchConfiguration o2) {
      MadlLaunchConfigWrapper wrapper1 = new MadlLaunchConfigWrapper(o1);
      MadlLaunchConfigWrapper wrapper2 = new MadlLaunchConfigWrapper(o2);
  ...
  public MadlRunAbstractAction(IWorkbenchWindow window, String name, int flags) {
    super(name, flags);
  ...
  protected void launch(ILaunchConfiguration config, UIInstrumentationBuilder instrumentation) {
    ...
    try {
      ...
      if (config.getType().getIdentifier().equals(MadlDebugCorePlugin.MADLIUM_LAUNCH_CONFIG_ID)) {
        MadlLaunchConfigWrapper launchConfig = new MadlLaunchConfigWrapper(config);
        DebugInstrumentationUtilities.recordLaunchConfiguration(launchConfig, instrumentation);

        launchConfig.markAsLaunched();
        LaunchUtils.clearMadliumConsoles();
      }
      ...
      MadlDebugCorePlugin.logError(e);
