...
import edu.depaul.cdm.madl.tools.ui.actions.ActionMessages;
...
/**
 * Common base class for action contributors for Madl editors.
 */
public class BasicMadlEditorActionContributor extends BasicTextEditorActionContributor {
...
//ss
  ...
  public BasicMadlEditorActionContributor() {
    super();

    ResourceBundle b = MadlEditorMessages.getBundleForConstructedKeys();
   ...
   // fToggleMarkOccurrencesAction = new ToggleMarkOccurrencesAction();
    ...
    fGotoMatchingBracket.setActionDefinitionId(MadlEditorActionDefinitionIds.GOTO_MATCHING_BRACKET);

    fShowOutline = new RetargetTextEditorAction(
        MadlEditorMessages.getBundleForConstructedKeys(),
        "ShowOutline."); //$NON-NLS-1$
    fShowOutline.setActionDefinitionId(MadlEditorActionDefinitionIds.SHOW_OUTLINE);

    fOpenDeclaration = new RetargetTextEditorAction(ActionMessages.getBundle(), "OpenAction_"); //$NON-NLS-1$
    fOpenDeclaration.setActionDefinitionId(MadlEditorActionDefinitionIds.OPEN_EDITOR);
    fOpenDeclaration.setId(MadlEditorActionDefinitionIds.OPEN_EDITOR);

    fOpenHierarchy = new RetargetTextEditorAction(
        MadlEditorMessages.getBundleForConstructedKeys(),
        "OpenHierarchy."); //$NON-NLS-1$
    fOpenHierarchy.setActionDefinitionId(MadlEditorActionDefinitionIds.OPEN_HIERARCHY);

    fOpenCallHierarchy = new RetargetTextEditorAction(
        MadlEditorMessages.getBundleForConstructedKeys(),
        "OpenCallHierarchy."); //$NON-NLS-1$
    fOpenCallHierarchy.setActionDefinitionId(MadlEditorActionDefinitionIds.ANALYZE_CALL_HIERARCHY);

    findDeclarations = new RetargetTextEditorAction(
        MadlEditorMessages.getBundleForConstructedKeys(),
        "FindDeclarations."); //$NON-NLS-1$
    findDeclarations.setActionDefinitionId(MadlEditorActionDefinitionIds.SEARCH_DECLARATIONS_IN_WORKSPACE);

    findOverrides = new RetargetTextEditorAction(
        MadlEditorMessages.getBundleForConstructedKeys(),
        "FindOverrides."); //$NON-NLS-1$
    findOverrides.setActionDefinitionId(MadlEditorActionDefinitionIds.SEARCH_DECLARATIONS_IN_HIERARCHY);

    findReferences = new RetargetTextEditorAction(
        MadlEditorMessages.getBundleForConstructedKeys(),
        "FindReferences."); //$NON-NLS-1$
    findReferences.setActionDefinitionId(MadlEditorActionDefinitionIds.SEARCH_REFERENCES_IN_WORKSPACE);

    fOpenStructure = new RetargetTextEditorAction(
        MadlEditorMessages.getBundleForConstructedKeys(),
        "OpenStructure."); //$NON-NLS-1$
    fOpenStructure.setActionDefinitionId(MadlEditorActionDefinitionIds.OPEN_STRUCTURE);

    fStructureSelectEnclosingAction = new RetargetTextEditorAction(b, "StructureSelectEnclosing."); //$NON-NLS-1$
    fStructureSelectEnclosingAction.setActionDefinitionId(MadlEditorActionDefinitionIds.SELECT_ENCLOSING);
    fStructureSelectNextAction = new RetargetTextEditorAction(b, "StructureSelectNext."); //$NON-NLS-1$
    fStructureSelectNextAction.setActionDefinitionId(MadlEditorActionDefinitionIds.SELECT_NEXT);
    fStructureSelectPreviousAction = new RetargetTextEditorAction(b, "StructureSelectPrevious."); //$NON-NLS-1$
    fStructureSelectPreviousAction.setActionDefinitionId(MadlEditorActionDefinitionIds.SELECT_PREVIOUS);
    fStructureSelectHistoryAction = new RetargetTextEditorAction(b, "StructureSelectHistory."); //$NON-NLS-1$
    fStructureSelectHistoryAction.setActionDefinitionId(MadlEditorActionDefinitionIds.SELECT_LAST);

    fGotoNextMemberAction = new RetargetTextEditorAction(b, "GotoNextMember."); //$NON-NLS-1$
    fGotoNextMemberAction.setActionDefinitionId(MadlEditorActionDefinitionIds.GOTO_NEXT_MEMBER);
    fGotoPreviousMemberAction = new RetargetTextEditorAction(b, "GotoPreviousMember."); //$NON-NLS-1$
    fGotoPreviousMemberAction.setActionDefinitionId(MadlEditorActionDefinitionIds.GOTO_PREVIOUS_MEMBER);

    fRemoveOccurrenceAnnotationsAction = new RetargetTextEditorAction(
        b,
        "RemoveOccurrenceAnnotations."); //$NON-NLS-1$
    fRemoveOccurrenceAnnotationsAction.setActionDefinitionId(MadlEditorActionDefinitionIds.REMOVE_OCCURRENCE_ANNOTATIONS);
  }
  ...
  public void contributeToMenu(IMenuManager menu) {
    ...
    if (editMenu != null) {
      ...
      //    MadlEditorMessages.ExpandSelectionMenu_label, "expandSelection"); //$NON-NLS-1$
...
//ss
   /* bars.setGlobalActionHandler(
        ...
        fTogglePresentation);
    bars.setGlobalActionHandler(
        MadlEditorActionDefinitionIds.TOGGLE_MARK_OCCURRENCES,
  ...
  public void setActiveEditor(IEditorPart part) {
   ...
   // fTogglePresentation.setEditor(textEditor);
    ...
    fShowOutline.setAction(getAction(textEditor, MadlEditorActionDefinitionIds.SHOW_OUTLINE));
    fOpenCallHierarchy.setAction(getAction(
        textEditor,
        MadlEditorActionDefinitionIds.ANALYZE_CALL_HIERARCHY));
    findDeclarations.setAction(getAction(
        textEditor,
        MadlEditorActionDefinitionIds.SEARCH_DECLARATIONS_IN_WORKSPACE));
    findOverrides.setAction(getAction(
        textEditor,
        MadlEditorActionDefinitionIds.SEARCH_DECLARATIONS_IN_HIERARCHY));
    findReferences.setAction(getAction(
        textEditor,
        MadlEditorActionDefinitionIds.SEARCH_REFERENCES_IN_WORKSPACE));
    fOpenHierarchy.setAction(getAction(textEditor, MadlEditorActionDefinitionIds.OPEN_HIERARCHY));
    fOpenStructure.setAction(getAction(textEditor, MadlEditorActionDefinitionIds.OPEN_STRUCTURE));
...
//ss
 /*   if (part instanceof MadlEditor) {
      MadlEditor javaEditor = (MadlEditor) part;
    ...
    //ss
    //action = getAction(textEditor, IMadlEditorActionConstants.COPY_QUALIFIED_NAME);
    MadlX.todo();
