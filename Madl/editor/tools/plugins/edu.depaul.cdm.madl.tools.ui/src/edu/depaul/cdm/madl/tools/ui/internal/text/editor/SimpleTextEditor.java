...
// import
// edu.depaul.cdm.madl.tools.ui.internal.text.editor.saveactions.RemoveTrailingWhitespaceAction;
...
/**
 * An editor for all files that are non-HTML and non-Madl. We're overriding the default text editor
 * because we want to control the context menu contributions.
 */
public class SimpleTextEditor extends TextEditor {

  public static final String ID = "edu.depaul.cdm.madl.tools.ui.text.editor.TextEditor"; //$NON-NLS-1$

  //ss
  //private RemoveTrailingWhitespaceAction removeTrailingWhitespaceAction;

  public SimpleTextEditor() {
    setRulerContextMenuId("#MadlSimpleTextEditorRulerContext"); //$NON-NLS-1$
  }
  ...
  protected void doSetInput(IEditorInput input) throws CoreException {
    ...
    if (descriptor != null && !descriptor.getId().equals(ID)) {
      ...
      Display.getDefault().asyncExec(new Runnable() {
        ...
        public void run() {
          try {
            IDE.openEditor(MadlToolsPlugin.getActivePage(), file, false);
  ...
  private void performSaveActions() {
    if (isRemoveTrailingWhitespaceEnabled()) {
      ...
      //ss
      /*
       * try { removeTrailingWhitespaceAction.run(); } catch (InvocationTargetException e) {
       * MadlToolsPlugin.log(e); }
       */
    }
  }
