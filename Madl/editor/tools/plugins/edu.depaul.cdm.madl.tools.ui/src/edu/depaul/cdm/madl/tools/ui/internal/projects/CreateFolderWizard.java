...
// import edu.depaul.cdm.madl.tools.core.pub.RunPubJob;
...
public class CreateFolderWizard extends BasicNewFolderResourceWizard {
   ...
   */
  public static final String WIZARD_ID = "edu.depaul.cdm.madl.tools.ui.new.folder"; //$NON-NLS-1$
  ...
  // http://pub.madllang.org/doc/pub-install.html
  ...
  private IContainer getPubWorkingDir(IFolder folder) {
    ...
    while (container != null) {
      if (container.findMember(MadlCore.PUBSPEC_FILE_NAME) != null) {
        return container;
      }
  ...
  private boolean isLibDirectory(IFolder folder) {
    if (folder.getName().equals("lib")
        && (folder.getParent().findMember(MadlCore.PUBSPEC_FILE_NAME) != null)) {
      return true;
    }
    return false;
  }
